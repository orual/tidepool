...
mod tests {
    ...
    fn test_value_construction() {
        ...
        match lit { Value::Lit(_) => (), _ => panic!("Expected Lit") }
        match con { Value::Con(_, _) => (), _ => panic!("Expected Con") }
        match closure { Value::Closure(_, _, _) => (), _ => panic!("Expected Closure") }
        match thunk { Value::ThunkRef(_) => (), _ => panic!("Expected ThunkRef") }
        match join { Value::JoinCont(_, _, _) => (), _ => panic!("Expected JoinCont") }
    }
